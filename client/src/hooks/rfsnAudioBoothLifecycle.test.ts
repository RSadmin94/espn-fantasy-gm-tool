...
describe("RFSN live-draft lifecycle sequences", () => {
  it("unlock → line 1 plays → ends → line 2 arrives → line 2 auto-plays", async () => {
    ...
    act(() => last().emit("ended"));
    // Written min dwell still applies after audio ends early.
    act(() => vi.advanceTimersByTime(commentaryDisplayMs(line1.text, true) + 50));
    settleBooth();
  ...
  it("new frame while speaking does not stop the active clip until it ends", async () => {
    const first = mkCard("coach", "p9:coach:primary", "Still speaking this line for the booth.");
    const next = mkCard("sofia", "p10:sofia:primary", "Next frame waits its turn.");
    const { result, rerender } = renderHook(useBoothWithAudio, {
      initialProps: {
        snapshot: snap("9.01", first),
        tts: true,
        audioStatus: audioStatus("pick-9", 9, [{ commentaryId: first.id }]),
      },
    });
    act(() => result.current.audio.unlockAudio());
    settleBooth();
    ...
    expect(result.current.booth.activeCard?.text).toBe(first.text);

    rerender({
      snapshot: snap("10.01", next),
      tts: true,
      audioStatus: audioStatus("pick-10", 10, [{ commentaryId: next.id }]),
    });
    ...
    settleBooth();
    await flushPlayback();
    expect(result.current.booth.activeCard?.text).toBe(next.text);
  });
