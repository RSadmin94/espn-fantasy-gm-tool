...
async function settle(): Promise<void> {
  ...
  // the 1.2s retry and 6s text-dwell windows so no timer-driven advance fires early.
  for (let i = 0; i < 3; i++) {
...
/** After audio `ended`, written min dwell may still be open — advance past it. */
async function settleAfterAudioEnded(): Promise<void> {
...
async function runOnce(run: number): Promise<RunResult> {
  ...
  try {
    ...
    await settleAfterAudioEnded(); // A ends -> booth advances -> Card B registered (pending)
    ...
    await settleAfterAudioEnded(); // B ends -> Card C (wrap-up) registered

    if (MockAudio.instances[2]) act(() => MockAudio.instances[2].emit("ended"));
    await settleAfterAudioEnded(); // C ends -> standby -> complete
