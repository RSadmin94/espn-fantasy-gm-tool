...
const GOLD="#f5c65a", TEAL="#35d3c5", MUTED="#91a6bf", RED="#ef5b5b", ORANGE="#f7902f", GREEN="#66d189", BLUE="#55a7ff", TEXT="#f3f8ff";
const PANEL: React.CSSProperties = { background:"linear-gradient(180deg,rgba(17,36,59,.97),rgba(9,24,42,.96))", border:"1px solid rgba(155,176,205,.22)", borderRadius:15 };
const SUB: React.CSSProperties = { background:"rgba(5,14,26,.34)", border:"1px solid rgba(155,176,205,.15)", borderRadius:10 };
const PAGEBG: React.CSSProperties = { background:"radial-gradient(circle at 76% -10%,rgba(53,211,197,.16),transparent 35%),radial-gradient(circle at 45% 110%,rgba(245,198,90,.08),transparent 34%),linear-gradient(180deg,#081426,#06111f)", color:TEXT };
    ...
    <div style={PAGEBG} className="-m-4 md:-m-6 p-5 md:p-7 min-h-full">
      <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button onClick={()=>draftQ.refetch?.()} className="px-3 py-2.5 rounded-[10px] text-[13px] font-extrabold inline-flex items-center gap-2" style={{border:"1px solid rgba(155,176,205,.22)",background:"rgba(16,34,56,.72)",color:MUTED}}><RefreshCw className="h-3.5 w-3.5"/>Refresh</button>
        </div>
      </div>
        ...
        <div style={PANEL} className="overflow-hidden">
          <div className="p-5">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="px-2 py-1.5 rounded-lg text-xs font-extrabold whitespace-nowrap" style={{background:"rgba(53,211,197,.10)",border:"1px solid rgba(53,211,197,.33)",color:TEAL}}>{pulse.length} signals</span>
            </div>
        ...
        <div style={PANEL} className="overflow-hidden"><div className="p-[18px]">
          ...
          <div className="mt-3">
            ...
            {pulse.map((p:any,i:number)=>(
              <div key={i} className="grid items-center gap-2 h-9 text-sm" style={{gridTemplateColumns:"26px 1fr 58px",borderTop:"1px solid rgba(155,176,205,.12)"}}>
        ...
        <div style={PANEL} className="overflow-hidden"><div className="p-[18px]">
          ...
          <div className="grid grid-cols-2 gap-2.5 mt-4">
            {[{b:"League",s:d.ok?"Synced":"\u2014"},{b:"Draft",s:(d.totalPicks??0)>0?"Live":"Indexed"},{b:"Owners",s:meters.length?`${meters.length} read`:"\u2014"},{b:"AI Memo",s:d.confidenceDashboard?"Ready":"\u2014"}].map((x:any,i:number)=>(
              <div key={i} style={SUB} className="p-3"><b className="block mb-1">{x.b}</b><span className="inline-block px-2 py-1 rounded-lg text-xs font-extrabold" style={{background:"rgba(53,211,197,.10)",border:"1px solid rgba(53,211,197,.33)",color:TEAL}}>{x.s}</span></div>
            ))}
          </div>
        </div></div>
      </div>
          ...
          <div className="mt-3 space-y-2.5">
            {actions.map((a:any,i:number)=>(
              <Link key={i} to={a.to} className="grid items-center gap-2.5 no-underline" style={{gridTemplateColumns:"34px 1fr 78px",...SUB,padding:"8px 10px",minHeight:60,color:TEXT}}>
                ...
                <span className="text-center text-xs font-extrabold rounded-md px-2 py-1.5" style={{border:"1px solid rgba(53,211,197,.35)",background:"rgba(53,211,197,.08)",color:TEAL}}>{a.cta}</span>
              </Link>
            ))}
          </div>
        </div></div>
        ...
        <div style={PANEL} className="overflow-hidden"><div className="p-[18px]">
          ...
          <div className="mt-2">
            ...
            {dnaOwners.map((m:any,i:number)=>{ const a=archetype(m); return (
              <div key={i} className="grid items-center gap-2.5 h-[50px]" style={{gridTemplateColumns:"36px 1fr 70px",borderTop:"1px solid rgba(155,176,205,.12)"}}>
        ...
        <div style={PANEL} className="overflow-hidden"><div className="p-[18px]">
          ...
          <div className="mt-3">
            ...
            {receipts.map((r:any,i:number)=>(
              <div key={i} className="grid items-center gap-3 h-[55px] text-sm" style={{gridTemplateColumns:"96px 1fr 74px",borderTop:"1px solid rgba(155,176,205,.12)"}}>
        ...
        <div style={PANEL} className="overflow-hidden"><div className="p-[18px]">
          ...
          <div className="mt-3">
            {readinessTable.map((t:any,i:number)=>(
              <div key={i} className="grid items-center h-7 text-sm" style={{gridTemplateColumns:"1fr 80px",borderTop:"1px solid rgba(155,176,205,.12)"}}>
...
function Pill({ children, gold }: any){
  return <span className="px-4 py-2.5 rounded-[10px] text-[13px] font-extrabold inline-flex items-center" style={ gold ? {color:GOLD,border:"1px solid rgba(245,198,90,.46)",background:"rgba(245,198,90,.10)"} : {border:"1px solid rgba(155,176,205,.22)",background:"rgba(16,34,56,.72)",color:TEXT} }>{children}</span>;
}
