...
function ArticleReader({ article, onClose, leagueName }: { article: Article; onClose: () => void; leagueName: string }) {
  const cfg = ARTICLE_TYPE_CFG[article.articleType] ?? ARTICLE_TYPE_CFG.season_archive;
  const Icon = cfg.icon;
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="border-b-2 border-zinc-800 pb-6 mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-zinc-600 text-[10px]">· {leagueName || "League Wire"} · Season {article.season}</span>
            {article.isPredicted && (
        ...
        <div className="mt-8 p-3 rounded-lg border border-zinc-800/40 bg-zinc-900/20 flex items-start gap-2">
          ...
          <p className="text-[10px] text-zinc-600 leading-relaxed">
            Generated by League Wire AI from verified database records. All scores, records, and standings are sourced directly from the {leagueName || "League Wire"} database. No statistics were fabricated.
...
export function LeagueWire() {
  ...
  const feedQ = _trpc.leagueNewsroom.getNewsroomFeed.useQuery(
    withLeagueSalt({ limit: 30 }, leagueContextKey),
    { enabled: leagueKeyReady },
  );
  ...
  return (
    <div className="-m-4 md:-m-6 p-5 md:p-7 min-h-full text-zinc-100" style={{ background: "radial-gradient(60% 80% at 80% -10%, rgba(139,92,246,.10), transparent 42%), #130e16" }}>
      ...
      {openArticle && <ArticleReader article={openArticle} onClose={() => setOpenArticle(null)} leagueName={leagueName} />}
        ...
        <div className="px-0 pb-4">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#8b97a8] uppercase tracking-[0.2em] font-bold mt-1">{leagueName ? `${leagueName} · ` : ""}Official Newsroom</p>
            </div>
          </div>
