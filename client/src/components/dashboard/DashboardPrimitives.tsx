...
const accentRing: Record<"none" | "gold" | "red" | "green" | "blue", string> = {
  none: "border-border shadow-sm shadow-black/20",
...
export function DashboardCard({
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-xl border bg-card/90 backdrop-blur-sm",
        accentRing[accent],
        className,
      )}
    >
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-border">
      <table className="w-full min-w-[280px] text-left">
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} className="border-b border-border last:border-0 hover:bg-foreground/[0.02]">
              {r.map((cellContent, j) => (
