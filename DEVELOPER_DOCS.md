...
**Trade Finder (RFSN-061 / 061A / 061B)** lives on the same page (`Trade Finder` tab). It is a league-aware recommendation engine: need/surplus → trade-priority partner discovery → bounded 1-for-1 / 2-for-1 / 1-for-2 / 2-for-2 generation → deterministic scoring → partner-rationality gate. Roster needScore is unchanged; discovery uses `tradePriorityScore = needScore * multiplier` (K/DST 0.20 unless targeted). Partner rationality rejects fair 2-for-1s that wreck the other starting lineup without a compensating need/hole/injury/depth/premium. AI may only explain ranked results; it cannot change values or order. Authority: `server/tradeFinder/`. Endpoint: `tradeFinder.find`.
