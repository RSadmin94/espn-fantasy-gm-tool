...
export const TIMEOUTS = {
  ...
  smokeTotal: 180_000,
  /** Full 196-pick Turbo draft with occasional broadcast holds — 20 minutes. */
  fullCertTotal: 1_200_000,
} as const;

/** Booth Replay only — never match "Replay same seed" which resets the draft. */
export function boothReplayButton(page: Page) {
  return page.locator("[data-rfsn-warroom-broadcast]").getByRole("button", { name: /^Replay$/i });
}
...
export function installAudioCounterInitScript(context: BrowserContext): Promise<void> {
  return context.addInitScript(() => {
    ...
    window.Audio = function (this: HTMLAudioElement, src?: string) {
      ...
      // CERT-004B: retain the detached instance so the cert observes real
      // currentTime/paused/duration/ended instead of document.querySelector("audio").
      (window as unknown as { __certLastAudio?: HTMLAudioElement }).__certLastAudio = a;
