/**
 * RFSN-054C Production visual smoke — wrap/overflow only.
 * Surfaces: Owner Dossier, GM Advisor, Historical Gallery, Live Draft, Transactions.
 * Usage: npx tsx scripts/_rfsn054c_production_smoke.mts
 */
...
const OUT = path.resolve("audit-artifacts/rfsn-054c/screenshots-production");
const WIDTH = 1440;
const PAGES = [
...
async function main() {
  ...
  const notes: string[] = ["env=production", "gate=wrap/overflow only", `viewport=${WIDTH}`];
  try {
    ...
    for (const [route, name] of PAGES) {
      ...
      if (name === "owners") {
        ...
        await page.waitForTimeout(2500);
      } else if (name === "draft-live") {
        await page.waitForSelector("[data-live-draft-control]", { timeout: 45_000 }).catch(() => null);
        await page.waitForTimeout(2500);
      } else if (name === "advisor") {
        await page.waitForSelector("textarea, [data-advisor-thread]", { timeout: 45_000 }).catch(() => null);
      ...
      notes.push(`${name}: ${JSON.stringify({ ...meta, wraps })}`);
      ...
      if (name === "owners") {
        ...
        if (clicked) {
          ...
          notes.push(`dossier: ${JSON.stringify({ ...dmeta, wraps: dwraps })}`);
