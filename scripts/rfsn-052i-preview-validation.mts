/**
 * RFSN-052I — Live Preview validation for career qualification + playoff semantics.
 *
 *   npx tsx scripts/rfsn-052i-preview-validation.mts
 *
 * ESPN 457622 only. Does not fabricate Sleeper/Workbook. Does not touch Production.
 */
...
const OUT_MD = path.join(OUT_DIR, "RFSN-052I-live-semantics.md");
const OUT_JSON = path.join(OUT_DIR, "RFSN-052I-live-semantics.json");
...
function failuresFor(question: string, message: string): string[] {
  ...
  if (/lozell/i.test(question) && /championship/i.test(question) && !/most championships/i.test(question)) {
  ...
  if (/compare demetri/i.test(question)) {
    ...
    if (!/regular season/i.test(message) || !/playoffs?/i.test(message)) failures.push("compare not labeled RS vs playoffs");
  }

  if (/head-to-head/i.test(question)) {
    if (!/demetri/i.test(message) || !/lozell/i.test(message)) failures.push("follow-up H2H lost Demetri + LOZELL");
  ...
  if (/one-point losses/i.test(question) && !/deroux|one-point/i.test(a)) {
    failures.push("one-point losses missing expected metric/owner");
  }
  if (/50-point blowout|blowout wins/i.test(question) && !/graham|50/i.test(a)) {
    failures.push("blowout answer missing expected metric/owner");
  }

  if (/best career winning percentage/i.test(question)) {
    if (/reginald sellers/i.test(message)) failures.push("short-tenure Reginald Sellers won leaderboard");
    if (!/median|games|season/i.test(message)) failures.push("qualification rule not stated");
    if (/does not have recorded career records/i.test(message)) failures.push("false missing-dataset");
  }

  if (/worst career record/i.test(question)) {
    if (/orlando howard/i.test(message) && /\b13\b/.test(message)) {
      failures.push("short-tenure orlando howard still on worst-career leaderboard");
    }
    if (!/median|games|season/i.test(message)) failures.push("qualification rule not stated");
    if (/does not have recorded career records/i.test(message)) failures.push("false missing-dataset");
  }

  if (/how good was reginald/i.test(question)) {
    if (!/reginald/i.test(message)) failures.push("named owner missing");
    if (!/10–3–0|10-3-0/.test(message)) failures.push("expected 10–3–0 record");
  }

  if (/playoff eliminations|playoff villain/i.test(question)) {
    const proven = /championship[-\s]?bracket/i.test(message);
    const honestWins = /recorded playoff wins/i.test(message);
    if (!proven && !honestWins) failures.push("neither championship-bracket nor recorded-playoff-wins label");
    if (/most playoff eliminations inflicted/i.test(message) && !proven) {
      failures.push("called eliminations without championship-bracket proof");
    }
    if (/does not have recorded/i.test(message) && /none recorded/i.test(a)) {
      failures.push("false missing-dataset");
    }
  }
...
async function main() {
  ...
  console.log("\nPreview ESPN 457622 052I gate PASS. Do not promote to Production.");
}
