  ...
  return result.trades
    .map((t, i) => {
      return [
        ...
        `rationality=${t.partnerRationality}`,
        `verdict=${t.rivalsVerdict}`,
      ...
      return memCache(cacheKey, 90_000, async () => {
        ...
        if (!payload) {
          return {
            ...
            emptyExplanation: "Rivals couldn't construct a valid trade from the current roster data. This season is not synced.",
            userNeeds: [],
            userSurplus: [],
            tradePriority: [],
            trades: [],
            ...
            metrics: {
              ...
              candidatesGenerated: 0, candidatesScored: 0, candidatesHardRejected: 0,
              candidatesRejectedByRationality: 0,
              tier1Count: 0, tier2Count: 0, tier3Count: 0, tier4Count: 0,
              candidatesReturned: 0, elapsedMs: 0,
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You explain already-ranked fantasy trades. Use ONLY the fact packet. Do not change rankings, classification, tier, values, fairness, or invent acceptance odds, psychology, urgency, or players. Return JSON array of {index, why, risk} matching the fact packet order. Each field ≤ 280 characters.",
                },
                {
                  role: "user",
