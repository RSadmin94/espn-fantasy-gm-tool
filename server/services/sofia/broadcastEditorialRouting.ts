...
export function roxanneEligible(moment: BroadcastMoment): boolean {
  ...
  // Clear reaches / steals at major+ — rivalry optional; still selective (not every ADP wiggle).
  if (
...
export function buildEditorialAssignment(
  ...
  if (planId === "value_pick") {
    ...
    // Major/historic steals may add Roxanne as entertainment bench — never as value lead.
    if (roxanneEligible(moment)) {
      basePlan = {
        ...basePlan,
        optionalVoices: ["roxanne"],
