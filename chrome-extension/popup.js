...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // ── P2: Weekly Player Stats ───────────────────────────────────────────────
  const MSG_SYNC_PLAYER_STATS = "GMWR_SYNC_PLAYER_STATS";

  function setStatsOut(text) {
    const el = document.getElementById("statsOut");
    if (el) el.textContent = text;
  }

  function parseSeasonInput() {
    const raw = (document.getElementById("statsSeasons")?.value || "").trim();
    if (!raw) return [2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025];
    return raw.split(",")
      .map(s => parseInt(s.trim(), 10))
      .filter(n => Number.isFinite(n) && n >= 2009 && n <= 2030);
  }

  document.getElementById("statsStatus")?.addEventListener("click", async () => {
    setStatsOut("Checking player stats cache status...");
    try {
      const r = await chrome.runtime.sendMessage({
        type: MSG_SYNC_PLAYER_STATS,
        seasons: [],
        statusOnly: true,
        skipExisting: false,
        leagueId: (document.getElementById("statsLeagueId")?.value || "457622").trim(),
      });
      if (!r?.ok && r?.error) { setStatsOut("Error: " + r.error); return; }
      const lines = ["── Player Stats Sync Report ───────────────────"];
      for (const s of (r?.seasons || [])) {
        lines.push(
          `  ${s.season}: ${s.weeksFetched} wks fetched  ${s.weeksSkipped} skipped  ${s.playerEntries} entries`
        );
      }
      lines.push(`\nTotal weeks: ${r?.totalWeeksFetched ?? 0}`);
      lines.push(`Total player entries: ${r?.totalPlayerEntries ?? 0}`);
      lines.push(`Cache writes: ${r?.totalCacheWrites ?? 0}`);
      setStatsOut(lines.join("\n"));
    } catch (e) {
      setStatsOut(e instanceof Error ? e.message : String(e));
    }
  });

  document.getElementById("statsSync")?.addEventListener("click", async () => {
    const lid = (document.getElementById("statsLeagueId")?.value || "457622").trim();
    const seasons = parseSeasonInput();
    if (seasons.length === 0) { setStatsOut("No valid seasons to sync."); return; }
    setStatsOut(
      `Syncing ${seasons.length} season(s): ${seasons.join(", ")}\nFetching from ESPN — may take several minutes. Do not close popup.`
    );
    try {
      const r = await chrome.runtime.sendMessage({
        type: MSG_SYNC_PLAYER_STATS,
      ...
      if (!r?.ok) { setStatsOut("Error: " + (r?.error || "unknown")); return; }
      const lines = ["── Sync Complete ───────────────────────────────"];
      for (const s of (r.seasons || [])) {
        const note = s.error ? ` [${s.error}]` : "";
        lines.push(
          `  ${s.season}: ${s.weeksFetched}/${s.weeksAttempted} wks  ${s.playerEntries} entries  ${s.weeksSkipped} skipped  ${s.weeksFailed} failed${note}`
        );
      }
      lines.push(`\nWeeks fetched:   ${r.totalWeeksFetched}`);
      lines.push(`Player entries:  ${r.totalPlayerEntries}`);
      lines.push(`Cache writes:    ${r.totalCacheWrites}`);
      setStatsOut(lines.join("\n"));
    } catch (e) {
      setStatsOut(e instanceof Error ? e.message : String(e));
    }
