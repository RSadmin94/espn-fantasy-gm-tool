/* RFSN-031A SPIKE ONLY — temporary auto-inject copy of existing Draft Board Monitor IIFE.
 * Do not treat as production. Parser/adapters unchanged from standalone build.
 */
(function rfsn031aSpikeReaderBootstrap() {
  try {
    console.info('[rfsn-031a-spike] Injection bundle executing', { href: String(location.href), at: new Date().toISOString() });
    window.__RFSN_031A_SPIKE__ = Object.assign({}, window.__RFSN_031A_SPIKE__ || {}, {
      bundleExecuted: true,
...
(() => {
  ...
  function normName(s) {
    return String(s ?? "").toLowerCase().replace(/[.'â€™`]/g, "").replace(/\s+(jr|sr|ii|iii|iv|v)$/i, "").replace(/\s+/g, " ").trim();
  }
  ...
  function resolvePlayerHeadshotUrl(args) {
    ...
    if (size === "full") {
      return espnPlayerHeadshotUrl(args.espnPlayerId, { w: 200, h: 145 }) || sleeperPlayerHeadshotUrl(args.sleeperPlayerId, { size: "full" }) || null;
    }
  ...
  function enrichEspnPickIdentity(args) {
    ...
    const espnId = args.playerId || resolved.espnPlayerId || void 0;
    const candidates = [];
...
`;
  ...
  function renderCard(doc, p) {
    ...
    const candidates = p.headshotCandidates?.length ? p.headshotCandidates : p.headshotUrl ? [p.headshotUrl] : [];
  ...
  function renderHeadshotSlot(doc, playerName, candidates) {
  ...
  function renderHeadshotFallback(doc, playerName) {
    const initials = playerName.split(/\s+/).filter(Boolean).map((w) => w[0]).join("").slice(0, 2).toUpperCase() || "?";
  ...
  function normToken(s) {
    return String(s ?? "").toLowerCase().replace(/[.'â€™`]/g, "").replace(/\s+(jr|sr|ii|iii|iv|v)$/i, "").replace(/[^a-z0-9]+/g, "").trim();
  }
  ...
  var _EspnBookmarkletPublisher = class _EspnBookmarkletPublisher {
    constructor(opts = {}) {
      ...
      this.recentBatches = [];
      /** Monotonic per armed session â€” stamped on every outbound batch. */
    ...
    /**
     * Phase 4 â€” idempotent reconciliation after War Room reconnect.
     * afterOverallPick <= 0 â†’ full board as baseline (no live notify).
     * afterOverallPick > 0 â†’ only newer picks as liveNotify candidates.
     */
     ...
     * First successful snapshot â†’ baseline projection batch (liveNotify=false).
     * Later new picks â†’ delta batches (liveNotify=true).
  ...
  var MonitorController = class {
    ...
    /** Phase 1/2 â€” expose publisher for ARM tests and extension handshake. */
    getEspnPublisher() {
  ...
  function isDevInjectAllowed(win) {
    try {
      if (win.localStorage?.getItem("ffrBoardMirrorDevInject") === "1") return true;
    } catch {
    }
    try {
      const q = new URL(win.location.href).searchParams.get("devInject");
      if (q === "1") return true;
    } catch {
    }
    return true;
  }
  if (isDevInjectAllowed(window)) {
    console.info(
      "[DraftBoardMonitor] Developer inject path. Prefer Live Draft \u2192 Open Board Mirror (/draft/board/:sessionId)."
    );
    startDraftBoardMonitor({ preferPopup: true, pollMs: 1e3 });
  } else {
    console.warn(
      "[DraftBoardMonitor] Inject blocked. Set localStorage.ffrBoardMirrorDevInject='1' for developer mode."
    );
  }
})();
