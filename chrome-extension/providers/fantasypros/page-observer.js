/**
 * RFSN-030C — FantasyPros solo mock page-world observer (v1.13).
 ...
 *
 * Accounting (session since ARM):
 * - rowsScanned: drafted rows examined in the latest poll
 * - picksEmitted: new picks sent as PICK_BATCH (not baselines)
 * - duplicatesSuppressed: known keys skipped (baseline + already emitted)
 * - picksObserved: alias of picksEmitted (compat only — do not use for inflation)
 */
(function fantasyProsPageObserver() {
  ...
  var diagnostics = {
    ...
    /** @deprecated alias of picksEmitted — kept for older popup/FFR readers */
    picksObserved: 0,
    observerRestarts: 0,
    lastEventAt: null,
    lastError: null,
  ...
  function syncCompatAlias() {
    diagnostics.picksObserved = diagnostics.picksEmitted;
  }

  function resetSessionDiagnostics() {
    diagnostics.rowsScanned = 0;
    diagnostics.picksEmitted = 0;
    diagnostics.duplicatesSuppressed = 0;
    diagnostics.picksObserved = 0;
  ...
  function establishBaseline(snap, providerDraftId) {
    ...
    diagnostics.rowsScanned = scanned;
    // Existing picks are baselined silently — not emitted, counted as suppressed.
    diagnostics.duplicatesSuppressed = baselineKeys.size;
    diagnostics.picksEmitted = 0;
    syncCompatAlias();
  }
  ...
  function tick() {
    ...
    diagnostics.rowsScanned = scanned;
    // Stable session total of known-but-not-new rows (baseline + prior emits already in set).
    diagnostics.duplicatesSuppressed = Math.max(diagnostics.duplicatesSuppressed, skippedKnown);
  ...
  function disarm() {
    ...
    if (pollTimer) clearTimeout(pollTimer);
    pollTimer = null;
    // Keep last session counters visible until next ARM (popup/FFR diagnostics).
