    ...
    return {
      ...
      diagnostics:
        data.diagnostics && typeof data.diagnostics === "object"
          ? {
              rowsScanned: Number(data.diagnostics.rowsScanned) || 0,
              picksEmitted: Number(data.diagnostics.picksEmitted) || 0,
              duplicatesSuppressed: Number(data.diagnostics.duplicatesSuppressed) || 0,
              // Alias: never prefer legacy picksObserved when picksEmitted is present.
              picksObserved:
                Number(data.diagnostics.picksEmitted) ||
                Number(data.diagnostics.picksObserved) ||
                0,
